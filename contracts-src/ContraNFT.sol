...
contract ContraNFT is ERC721, Ownable, ReentrancyGuard {
    ...
    function mint() external nonReentrant {
        ...
        // CEI pattern: external calls first, then state changes
        ...
        // State change after all external calls (checks-effects-interactions)
        totalMinted = tokenId;
    ...
    function setMaxSupply(uint256 _newMax) external onlyOwner notPaused {
    ...
    function setPaymentToken(address _newToken) external onlyOwner notPaused {
    ...
    function setMintPrice(uint256 _newPrice) external onlyOwner notPaused {
    ...
    function setTreasury(address _newTreasury) external onlyOwner notPaused {
        require(_newTreasury != address(0), "zero treasury");
        emit TreasuryUpdated(treasury, _newTreasury);
        treasury = _newTreasury;
    }

    function setBeneficiary(address _newBeneficiary) external onlyOwner notPaused {
